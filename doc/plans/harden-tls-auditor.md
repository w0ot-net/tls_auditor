...
- Parse bare hosts, IPv4/hostname ports, and bracketed IPv6 ports consistently;
  keep the existing mixed-input behavior where a bare host in host-port mode
...
Use one host/port parser for both input endpoints and CSV `Host:Port` values.
Bracketed IPv6 must be stored without brackets internally, while formatting must
continue to emit `[address]:port`. A final numeric suffix is an explicit port
only for a normal hostname/IPv4 form or a bracketed IPv6 form; an unbracketed
IPv6 literal remains a bare host and receives port 443 in host-port mode. Invalid
bracket/port forms should produce a clear input error rather than being silently
reinterpreted as a hostname.
...
- `tests/test_tls_auditor.py`: add compact standard-library regression tests for
  endpoint parsing, TLS 1.1 classification/XML output, exact host/port grouping,
  sentinel-aware merging, and the relevant scan cleanup/failure invariant.
...
1. Add the shared endpoint parser and focused tests for hostname, IPv4, bare
   IPv6, bracketed IPv6, default-port, duplicate, and malformed input behavior;
   route host-port input and CSV parsing through it without changing CSV output.
...
4. Extract and test the shared result-row merge helper, then use it from both XML
   aggregation and CSV merge paths.
5. Perform the bounded code-hygiene pass: state Python 3.8+, remove unused
   imports/trailing whitespace, and add the cache-only `.gitignore`.
6. Rewrite the README from the final CLI behavior, including prerequisites,
...
- Hostname, IPv4, and bracketed IPv6 endpoint inputs round-trip to the correct
  `Host:Port` form, while malformed explicit endpoints fail clearly.
